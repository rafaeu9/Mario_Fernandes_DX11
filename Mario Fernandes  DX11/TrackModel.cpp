...
HRESULT TrackModel::LoadObjModel(char* filename)
{
	...
	hr = D3DX11CompileFromFile("track_shader.hlsl", 0, 0, "ModelVS", "vs_4_0", 0, 0, 0, &VS, &error, 0);
	...
	hr = D3DX11CompileFromFile("track_shader.hlsl", 0, 0, "ModelPS", "ps_4_0", 0, 0, 0, &PS, &error, 0);
	if (error != 0)//check for shader compilation error
	{
		OutputDebugStringA((char*)error->GetBufferPointer());
		error->Release();
...
HRESULT TrackModel::AddTexture(char* filename)
{
	HRESULT hr = S_OK;
...
void TrackModel::LookAt_XZ(float x, float z)
{
...
void TrackModel::MovedFoward(float distance)
{
	m_x += sin(m_yAngle * (XM_PI / 180.0)) * distance;
	m_z += cos(m_yAngle * (XM_PI / 180.0)) * distance;
}

void TrackModel::CalculateModelCentrePoint()
{
...
void TrackModel::CalculateBoundingSphereRadius()
{
	float distance = 0;
...
XMVECTOR TrackModel::GetBoundingSphereWorldSpacePosition()
{
	XMMATRIX world;
	world  = XMMatrixScaling(m_scale, m_scale, m_scale);
	world *= XMMatrixRotationX(XMConvertToRadians(m_xAngle));
...
bool TrackModel::CheckCollision(Model* model)
{
	...
	float distance =  model->GetBoundingSphereRadius();

	for (int i = 0; i < m_pObject->numverts; i++)
	{
		if (distance > sqrt(pow(m_pObject->vertices[i].Pos.x - x2, 2) + pow(m_pObject->vertices[i].Pos.y - y2, 2) + pow(m_pObject->vertices[i].Pos.z - z2, 2)))
		{
			for (int u = 0; u < model->m_pObject->numverts; u++)
			{
				if (distance > sqrt(pow(m_pObject->vertices[i].Pos.x - model->m_pObject->vertices[u].Pos.x, 2) +
									pow(m_pObject->vertices[i].Pos.y - model->m_pObject->vertices[u].Pos.y, 2) +
									pow(m_pObject->vertices[i].Pos.z - model->m_pObject->vertices[u].Pos.z, 2)))
				{
					return true;
				}
			}
