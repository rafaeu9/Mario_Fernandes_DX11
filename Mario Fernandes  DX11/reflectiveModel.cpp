...
void reflectiveModel::LookAt_XZ(float x, float z)
{
...
void reflectiveModel::MovedFoward(float distance)
{
	m_x += sin(m_yAngle * (XM_PI / 180.0)) * distance;
	m_z += cos(m_yAngle * (XM_PI / 180.0)) * distance;
}

void reflectiveModel::CalculateModelCentrePoint()
{
...
void reflectiveModel::CalculateBoundingSphereRadius()
{
	float distance = 0;
...
XMVECTOR reflectiveModel::GetBoundingSphereWorldSpacePosition()
{
	XMMATRIX world;
	world = XMMatrixScaling(m_scale, m_scale, m_scale);
	world *= XMMatrixRotationX(XMConvertToRadians(m_xAngle));
